...
try {
    ...
    const otpPost=await axios.post('https://saylani-admission-final-backend.vercel.app/api/auth/verifyemail',{otp},{headers:{token:`Bearer ${Token}`}})
...
const otpResendHook=async(email,setIsLoding)=>{
    ...
    try {
        setIsLoding(()=>true)
        const resendOtp=await axios.post('https://saylani-admission-final-backend.vercel.app/api/auth/resendOtp',{email})
